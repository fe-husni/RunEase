...
export function InstallBanner() {
  ...
  return (
    <Card deco="yellow" className="relative mx-auto max-w-md border-4 p-4 sm:max-w-2xl">
      <button
        onClick={dismiss}
        aria-label="Tutup banner install"
        className="absolute right-2 top-2 flex h-8 w-8 items-center justify-center border-2 border-bauhaus-black bg-white text-xs font-black focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-bauhaus-blue"
      >
        <X className="h-4 w-4" />
      </button>
      <div className="flex gap-3 pr-6">
        ...
        <div className="min-w-0 flex-1">
          <div className="font-black uppercase tracking-tight">Install RunEase</div>
          {isIOS ? (
          ...
          {!isIOS && (
            <Button variant="blue" size="sm" shape="square" className="mt-3" onClick={prompt}>
              <Download className="mr-1 h-4 w-4" /> Install Sekarang
            </Button>
          )}
          <Button variant="ghost" size="sm" className="ml-2 mt-3" onClick={dismiss}>
            Nanti
          </Button>
        </div>
      </div>
